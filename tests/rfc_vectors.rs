...
/// Thumbprint tests (RFC 7638)
mod thumbprint {
    use super::*;

    /// RFC 7638 Section 3.1 - Example RSA Key Thumbprint
    ///
    /// This test verifies the exact thumbprint value from the RFC specification.
    #[test]
    fn test_rfc7638_example_thumbprint() {
    ...
    fn test_thumbprint_ignores_optional_fields() {
        // Same key with different optional fields should have same thumbprint
        let jwks1 = serde_json::from_str::<KeySet>(r#"{"keys": [{
            "kty": "RSA",
            "n": "0vx7agoebGcQSuuPiLJXZptN9nndrQmbXEps2aiAFbWhM78LhWx4cbbfAAtVT86zwu1RK7aPFFxuhDR1L6tSoc_BJECPebWKRXjBZCiFV4n3oknjhMstn64tZ_2W-5JsGY4Hc5n9yBXArwl93lqt7_RN5w6Cf0h4QyQ5v-65YGjQR0_FDW2QvzqY368QQMicAtaSqzs8KJZgnYb9c7d0zgdAZHzu6qMQvRL5hajrn1n91CbOpbISD08qNLyrdkt-bFTWhAI4vMQFh6WeZu0fM4lFd2NcRwr3XPksINHaQ-G_xBniIqbw0Ls1jF44-csFCur-kEgU8awapJzKnqDKgw",
            "e": "AQAB"
        }]}"#).unwrap();

        let jwks2 = serde_json::from_str::<KeySet>(r#"{"keys": [{
            "kty": "RSA",
            "kid": "my-key",
            "use": "sig",
            "alg": "RS256",
        ...
        let t1 = jwks1.first().unwrap().thumbprint();
        let t2 = jwks2.first().unwrap().thumbprint();
        assert_eq!(t1, t2);
    }
}
